...
object PBijection {
	...
	def identity[T]:PBijection[T,T]	= PBijection(
			Some.apply,
			Some.apply)
			
	def trivial[T]:PBijection[T,T]	= PBijection(
			_ => None,
			_ => None)
}
...
trait PBijection[S,T] { self =>
	...
	final def inverse:PBijection[T,S]	= PBijection(
			read, 
			write)
	...
	final def andThen[U](that:PBijection[T,U]):PBijection[S,U]	= PBijection(
