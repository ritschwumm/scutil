...
trait ApplicativeSyntax {
	...
	implicit class ApplicativeBooleanSyntax(peer:Boolean) {
		def ifA[F[_]](action:F[Unit])(implicit F:Applicative[F]):F[Unit]	= F.ifA(peer)(action)
	}

	implicit class ApplicativeOptionSyntax[T](peer:Option[T]) {
		def optionalA[F[_]](action:T=>F[Unit])(implicit F:Applicative[F]):F[Unit]	= F.optionalA(peer)(action)
	}

	implicit class ApplicativeValueSyntaxExt[F[_],T](peer:F[T])(implicit F:Applicative[F]) {
		def pa[U](func:F[T=>U]):F[U]					= (F ap peer)(func)
		def combine[U,V](that:F[U])(func:(T,U)=>V):F[V]	= (F combine (peer, that))(func)
	}

	/*
	implicit class ApplicativeArrowSyntaxExt[F[_],S,T](peer:F[S=>T])(implicit F:Applicative[F]) {
