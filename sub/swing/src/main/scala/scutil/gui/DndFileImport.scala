...
object DndFileImport {
	def install(target:JComponent, handler:PFunction[IntPoint,Effect[Validated[Exception,Nes[File]]]]):Disposable	= {
		target setTransferHandler new DndFileImport(handler)
		disposable { target setTransferHandler null }
	}
...
final class DndFileImport(handler:PFunction[IntPoint,Effect[Validated[Exception,Nes[File]]]]) extends TransferHandler {
	...
	private def importEffect(support:TransferSupport):Option[Effect[Validated[Exception,Nes[File]]]] =
			supportsFormat(support)	flatGuard
			handler(dropIntPoint(support))
			
	private def supportsFormat(support:TransferSupport):Boolean =
			support.getDataFlavors.toSet containsAny
			DndFileImport.importable
