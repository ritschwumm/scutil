...
class SourceLocationTest extends Specification {
	...
	"implicit SourceLocation" should {
		"be available with only an implicit parameter list" in {
			foo mustEqual SourceLocation(Some("sub/base/src/test/scala/scutil/lang/SourceLocationTest.scala"), "SourceLocationTest.scala", 11)
		}
		"be available with one normal and an implicit parameter list" in {
			bar(1) mustEqual SourceLocation(Some("sub/base/src/test/scala/scutil/lang/SourceLocationTest.scala"), "SourceLocationTest.scala", 14)
		}
		"be available with multiple normal and an implicit parameter list" in {
			quux(1)(1) mustEqual SourceLocation(Some("sub/base/src/test/scala/scutil/lang/SourceLocationTest.scala"), "SourceLocationTest.scala", 17)
		}
		
		"work with multiple implicit parameters" in {
			xxx mustEqual SourceLocation(Some("sub/base/src/test/scala/scutil/lang/SourceLocationTest.scala"), "SourceLocationTest.scala", 21)
		}
		"work with multiple implicit parameters" in {
			yyy mustEqual SourceLocation(Some("sub/base/src/test/scala/scutil/lang/SourceLocationTest.scala"), "SourceLocationTest.scala", 24)
		}
	}
